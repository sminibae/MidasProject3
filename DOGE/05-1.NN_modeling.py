...
def training(model_name, model_instance, seq_length, train_loader, valid_loader):
    ...
    for epoch in range(num_epochs):
        ...
        print(logs)
        with open(os.path.join(BASE_DIR, 'NN_modeling_logs.txt'),'a') as f:
            f.write(logs)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(BASE_DIR, 'Models/best_model_state_dict.pth'))
            torch.save(model, os.path.join(BASE_DIR, 'Models/best_model.pth'))
    ...
    torch.save(model.state_dict(), os.path.join(BASE_DIR, f'Models/{model_name}_model_state_dict_{seq_length}.pth'))
    torch.save(model, os.path.join(BASE_DIR, f'Models/{model_name}_model_{seq_length}.pth'))
    print(f'Saved {model_name}_{seq_length} model')

    with open(os.path.join(BASE_DIR, f'Models/{model_name}_history_{seq_length}.json'), 'w') as f:
        json.dump(history, f)
