...
def process_chunk(chunk, chunk_index):
    ...
    if len(chunk) >= 20:
        for i in range(len(chunk) - 19):
            matrix = chunk.drop(columns=['plus_6', 'minus_6', 'zero_6'], axis=1).iloc[i:i+40].values
            matrix_list.append(matrix)
            answer = chunk.iloc[i+39][['plus_6', 'minus_6', 'zero_6']].tolist()
    ...
    np.save(f'Data/matrix_array_40_{chunk_index}.npy', np.array(matrix_list))
    np.save(f'Data/answer_array_40_{chunk_index}.npy', np.array(answer_list))
